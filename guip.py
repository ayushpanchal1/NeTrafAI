...
class NetworkCaptureApp(QWidget):
    ...
    def initUI(self):
        ...
        self.prediction_output = QTextEdit(self)  # New Text Box
        self.prediction_output.setReadOnly(True)  # Set as read-only so users can't edit it
        #self.prediction_output.setStyleSheet("background-color: black; color: white;")
    ...
    def generate_pdf_report(self):
        ...
        pdf.cell(200, 10, txt="Malicious and Normal Event Counts", ln=True, align='L')
                ...
                with conn:
                    ...
                    while True:
                        ...
                        try:
                            ...
                            # Extract mal_count and nor_count
                            mal_count = message.get("mal_count", 0)
                            nor_count = message.get("nor_count", 0)

                            # Store the counts in variables (you can update the UI or use them elsewhere)
                            self.mal_count = mal_count
                            self.nor_count = nor_count

                            # Prepare the text message to append
        ...
        if self.packet_info:
            with self.packet_info_lock:
                packet_info_text = (f"Source IP: {self.packet_info.get('src_ip', 'N/A')}\n"
                                    ...
                                    f"Source Port: {self.packet_info.get('src_port', 'N/A')}\n"
                                    f"Destination Port: {self.packet_info.get('dst_port', 'N/A')}\n"
                                    f"Normal Count: {self.nor_count}\n"
                                    f"Malicious Count: {self.mal_count}")
